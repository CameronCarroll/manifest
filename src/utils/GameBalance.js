...
class GameBalance {
  ...
  simulateCombat(playerUnits, enemyUnits, iterations = 10) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      const totalDPS_player = playerUnits.reduce((total, unit) => 
        total + this.calculateDPS(unit), 0);
      const totalDPS_enemy = enemyUnits.reduce((total, unit) => 
        total + this.calculateDPS(unit), 0);
